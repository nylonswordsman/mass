 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this latest version adds a basic bad luck protection system, where
 * plugins can register that something is currently getting unlucky and change
 * things in the player's favor next time where the bad luck protection status
 * can then be cleared. A basic bad luck protection roll function was added
 * as well to automatically account for bad luck in success rolls.
 *
 * This version also added page up and down functionality for scrolling
 * windows, so they can now handle swapping what they display when these
 * default controls are pressed (if handled).
 *
 * A bug was fixed with the inline videos that could cause issues if the
 * video was changed before it was fully loaded.
 *
 * Version 1.37.0
...
var CGMZ = {};
CGMZ.Versions = {};
CGMZ.Versions["CGMZ Core"] = "1.37.0";
CGMZ.Core = {};
