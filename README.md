...
### Installation
Unfortunately, you can't just download this repo off of Github, extract it, and call it a day. Copies downloaded through Github can't be updated, and also won't launch. You'll have to clone the repo on your own device, but this is a quick, easy, and perfectly safe process.

#### Windows
1. Download [Git Bash (insert link!)](h), if you don't already have it installed.
2. Open Git Bash. Type `cd ~/Documents` and press enter.
* Make sure to not copy-paste or shift+insert! You'll have to type commands into Git Bash manually.
3. Type `git clone https://github.com/nylonswordsman/mass` and press enter.
4. Go to your Documents directory in File Explorer. `Game.exe` should be located in a folder called `mass`; run Game.exe to run the demo.

#### Linux
Installation on Linux works just about the same as on Windows, but you'll already have Git out-of-the-box in the base terminal and won't need to download Git Bash. You'll have to use a compatibility tool for the game to run, however. To do this:
1. Add `Game.exe` as a non-Steam game on Steam.
2. Right-click Game.exe in your library. Properties -> Compatability -> Force usage of specific compatability tool
3. Go down the list of tools and try running the game until one of them lets it launch.
...
1. Open Git Bash for Windows, or your terminal of choice for Linux.
2. `cd` to the location of the repo.
* If you were following the windows guide, the command should be `cd ~/Documents/mass`.
3. Type `git pull origin master` and press enter.
...
* an instrumental of *Kingdom* that can be found [here](https://youtu.be/EZej7poPqxs?si=atsH3SDUq4cPI58E) ripped and de-vocaled by Caritas (boss theme 1)
